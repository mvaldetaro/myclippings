...
export function getQuoteImageUrl(
  ...
  if (overrides?.showBookTitle !== undefined)
    params.set('showBookTitle', String(overrides.showBookTitle));
...
export async function downloadQuoteImage(
  ...
  const qs = params.toString();
  const url = qs
    ? `/api/quotes/${bookId}/${clipId}/download?${qs}`
    : `/api/quotes/${bookId}/${clipId}/download`;
